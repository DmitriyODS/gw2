...
- **Каналы уведомлений** (`MainActivity`) — id `messages`/`tasks`/
  ...
  обновление поверх).
- **Обновление обёртки** — два пути поверх общего `AppUpdater.java`
  (скачивание `/apps/mobile/groovework.apk` + системная установка поверх —
  подпись и applicationId неизменны): автопроверка `UpdateChecker` (раз в
  6 часов, диалог) и принудительная кнопка в «О приложении» без троттла
  (плагин `NativeShellPlugin`: getInfo/checkUpdate/installUpdate с прогрессом;
  он же красит системные панели под тему — `setSystemBars`, фронт дёргает
  при смене темы).
- **Ярлыки разделов на домашнем экране** — долгое нажатие по плитке «Пуска» →
  «На экран телефона»: `NativeShell.pinShortcut` предлагает лаунчеру закрепить
  ярлык (`ShortcutManager.requestPinShortcut`, подтверждает его сам лаунчер).
  Значок рисует ВЕБ-СЛОЙ (`utils/shortcutIcon.js` — глиф раздела в цветах
  текущей темы) и передаёт base64-PNG: копий иконок двадцати разделов в
  ресурсах не держим. Клик по ярлыку несёт путь раздела в extra `gw_path`:
  на холодном старте `MainActivity` грузит сразу его, в уже запущенном
  приложении шлёт фронту событие `gw:open-path` — перезагрузка страницы стёрла
  бы открытые разделы (ловит `desktop/shellCore.js`).
