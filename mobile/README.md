...
make apk         # npm install + cap sync + gradlew assembleRelease
                 #   → apps/mobile/groovework.apk (+ сверка versionCode)
...
- `/apps/mobile/groovework.apk` + `/apps/mobile/version.json` — ЭТО приложение.
  Сюда смотрят: карточка «Скачать APK» в «О приложении» на сайте, всплывашка
  устаревшего нативного приложения и автообновление самой обёртки.
- `/apps/groovework.apk` + `/apps/version.json` — ЗАМОРОЖЕННЫЙ канал прежнего
  нативного приложения (`GrooveWorkAndroid/`, `make apk-legacy`): там лежит
  переходная nag-сборка, которая показывает обязательную модалку «приложение
  устарело» (повтор каждые 10 минут) и ведёт на новый канал. Старые установки
  обновляются до неё штатной тихой проверкой. После миграции пользователей
  канал не трогать.
