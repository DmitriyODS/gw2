...
| Слой | Технология |
|---|---|
| Бэкенд | Go 1.26 · go-kit · Fiber · gRPC · pgx — микросервисы calls, auth, messenger, ai, groove, tasks, gateway (`back-go/<svc>`) |
...
Восемь Go-микросервисов (включая шлюз) над общей PostgreSQL (схему ВСЕХ таблиц ведёт run-once контейнер `migrate` — goose, baseline + инкременты в `back-go/migrate/migrations`) + Redis + LiveKit. Python/Flask ликвидирован полностью.
...
- **callsvc** (`back-go/calls`) — вся бизнес-логика звонков + LiveKit (токены/комнаты/вебхуки) + оркестрация плашки звонка в чате (gRPC msgsvc: EnsureDialog при p2p-старте, CreateCallMessage/GetCallMessage → события `message:new`/`message:updated` через свой Redis-канал). gRPC :9090 (ринг-фаза, зовёт gateway) и HTTP :8090 (REST `/api/calls/*`, вебхуки LiveKit, `/healthz`).
...
Роли фиксированы в БД, создавать/удалять нельзя. Константы: `domain.Level*` в Go-сервисах; фронт — `composables/usePermission.js` (уровень из `auth.user.role.level`).
...
```bash
./dev.sh             # одна команда: инфра в Docker + миграции + 7 Go-сервисов + Vite :5173
# или по частям:
...
- **Тесты:** Go — `go test ./...` в back-go/{calls,auth,messenger,ai,groove,tasks,gateway} (фейки портов, без БД/LiveKit; в tasks — паритет YouGile sync_hash/parser/apply, в gateway — miniredis + интеграционный WS-тест handshake/доставки).
