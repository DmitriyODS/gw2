...
| Слой | Технология |
|---|---|
| Бэкенд | Go 1.26 · go-kit · Fiber · gRPC · pgx — микросервисы calls, auth, messenger, ai, groove, tasks, gateway, push, mail, registry, calendar, diary (`back-go/<svc>`) |
...
back-go/     — Go-микросервисы: calls (звонки), auth (авторизация и пользователи),
               ...
               calendar (календари), diary (личные ежедневники),
...
Тринадцать Go-микросервисов (включая шлюз, push, mail, registry, calendar и diary) над общей PostgreSQL (схему ВСЕХ таблиц ведёт run-once контейнер `migrate` — goose, baseline + инкременты в `back-go/migrate/migrations`) + Redis + LiveKit. Python/Flask ликвидирован полностью.
...
- **diarysvc** (`back-go/diary`) — личные ежедневники: списки записей-задач, привязанных к дню. В отличие от компанийного календаря, ежедневник принадлежит ОДНОМУ пользователю (`diaries.owner_id`), НЕ зависит от компании (кросс-компанийный, как мессенджер; роут `/diaries` требует лишь авторизации, без активной компании) и имеет ФИКСИРОВАННЫЙ набор полей карточки (день `entry_date`, опц. время начала/конца `start_min/end_min` в минутах от полуночи, название, описание, отметка `done` → архив, связь `linked_task_id` с задачей tasksvc). Внутри ежедневника две вкладки: «Активные» (`done=false`, просмотр по дню/неделе/месяцу, по умолчанию НЕДЕЛЯ) и «Архив» (`done=true`). HTTP :8101 (REST `/api/diaries/*`): только владелец правит ежедневник и записи; запись можно отметить выполненной (`PATCH …/records/:rid/done`) и привязать к ней задачу+юнит (кнопка в карточке создаёт задачу через tasksvc-API и зовёт `PATCH …/records/:rid/link`). **Шаринг (read-only):** публичной ссылкой (`diary_shares`, код-capability; публичные `GET /api/diaries/shared/:code{,/records,/export}`, фронт-роут `/diary/:code` — `SharedDiaryView`) и адресно конкретному пользователю платформы (`diary_user_shares`; `…/members`) — у адресата ежедневник появляется во вкладке «Поделились» (раздел делится на «Мои»/«Поделились»). Скоуп по владельцу; межсервисных вызовов нет (авторизация локальная, PASETO); файлов нет. Сокет-события адресованы комнатам владельца и адресатов: `diary:*` (created/updated/deleted/shared/unshared) и `diary_entry:*` (имя с префиксом, чтобы не пересекаться с `entry:*` календаря) → Redis `gw2:diary:events`. Фронт — `DiaryView`, общий кастомный `TimePicker` (он же заменил браузерный выбор времени в расписании тёмной темы).
...
```bash
./dev.sh             # одна команда: инфра в Docker (+ mailpit) + миграции + 10 Go-сервисов + Vite :5173
# или по частям:
...
- **Тесты:** Go — `go test ./...` в back-go/{calls,auth,messenger,ai,groove,tasks,gateway,push,registry,calendar,diary} (фейки портов, без БД/LiveKit; в tasks — паритет YouGile sync_hash/parser/apply, в gateway — miniredis + интеграционный WS-тест handshake/доставки, в push — маршрутизация событий в пуши + presence-гейт, в registry — валидация значений/маски/вариантов, пересчёт search_text, чистка данных удалённого поля, скоуп по компании; в calendar — то же + обязательность event_at без секунд; в diary — скоуп по владельцу, read-only для адресатов, перенос done в архив, адресация сокет-событий владельцу и адресатам).
