...
| Слой | Технология |
|---|---|
| Бэкенд | Go 1.26 · go-kit · Fiber · gRPC · pgx — микросервисы calls, auth, messenger, ai, groove, tasks, gateway, push, mail, registry (`back-go/<svc>`) |
...
Одиннадцать Go-микросервисов (включая шлюз, push, mail и registry) над общей PostgreSQL (схему ВСЕХ таблиц ведёт run-once контейнер `migrate` — goose, baseline + инкременты в `back-go/migrate/migrations`) + Redis + LiveKit. Python/Flask ликвидирован полностью.
...
- **Тесты:** Go — `go test ./...` в back-go/{calls,auth,messenger,ai,groove,tasks,gateway,push,registry} (фейки портов, без БД/LiveKit; в tasks — паритет YouGile sync_hash/parser/apply, в gateway — miniredis + интеграционный WS-тест handshake/доставки, в push — маршрутизация событий в пуши + presence-гейт, в registry — валидация значений/маски/вариантов, пересчёт search_text, чистка данных удалённого поля, скоуп по компании).
