...
| API-документация | flasgger (Swagger UI на /apidocs — только Flask-эндпоинты: YouGile, presence) |
...
- **Flask-монолит** (`back/`, :5000 в docker / :5001 в dev) — YouGile-интеграция, Socket.IO (шлюз реального времени + presence) и Redis-мосты событий. От домена задач остался мини-слой для YouGile (см. раздел YouGile).
...
- **tasksvc** (`back-go/tasks`) — ядро платформы: задачи (избранное, личные цвета, комментарии, ответственный/этап), юниты (1 активный на пользователя), типы юнитов, этапы, отделы, статистика (common/extended/profile/user-tasks/responsibles/employees + xlsx-экспорт на excelize). HTTP :8095 (REST `/api/tasks|units|unit-types|departments|stages|stats`). Поиск задач: при включённом AI — целиком семантический через gRPC aisvc (SemanticSearch, fail-open в LIKE), реиндекс эмбеддингов — ReindexTask; хуки геймификации — gRPC groovesvc (OnUnitStarted/OnUnitStopped/OnTaskClosed, fire-and-forget). Сокет-события → Redis `gw2:tasks:events` (мост Flask); служебные `_yg_task_updated`/`_yg_task_archived` дёргают YouGile-пуш во Flask (до фазы 4).
- **groovesvc** (`back-go/groove`) — весь «Мой Groove» (см. раздел ниже). HTTP :8094 (REST `/api/groove/*`) и gRPC :9094 (хуки доменных событий: tasksvc — юниты/задачи, Flask — YouGile-вебхук, msgsvc — pet-чат). События → Redis `gw2:groove:events`; фоновые циклы заботы и AI — внутри.
...
- `front/src/api/auth.js`, `users.js`, `companies.js`, `roles.js`, `backup.js`, `ai.js` и `changelog.js` ведутся вручную (Go-сервисы Swagger не публикуют) — `npm run gen:api` их не трогает (MANUAL_TAGS в gen-api.mjs).
...
Последний крупный Flask-домен (уйдёт внутрь tasksvc в фазе 4). Домен задач уже в tasksvc, поэтому во Flask остался мини-слой: `integrations/yougile/task_dump.py` (enrich-дамп для ответов и сокет-событий), срезы `task_repo`/`comment_repo`/`department_repo`/`stage_repo` и `task_service.create_task` (только для импорта). Исходящий пуш в YG при правках задач дёргает tasksvc — служебными событиями `_yg_task_updated`/`_yg_task_archived` в `gw2:tasks:events` (мост диспатчит их в `integrations/yougile/bridge_hooks.py`, best-effort). Per-company настройка (Руководитель подключает компанию/проект/доску; ключи пользователей и компаний шифруются Fernet — `YOUGILE_ENC_KEY`, `AI_KEY_ENCRYPTION_KEY` в env; ротация ключей = потеря привязок). Импорт карточек по короткой ссылке (поиск включает BFS по подзадачам), экспорт задач, двусторонняя синхра через вебхук `POST /api/yougile/webhook/<companyId>/<secret>` (нужен `YOUGILE_WEBHOOK_PUBLIC_BASE`). Обмен с YG — в фоне, не блокирует API.
...
- **Тесты:** Go — `go test ./...` в back-go/{calls,auth,messenger,ai,groove} (фейки портов, без БД/Redis/LiveKit). Python — `cd back && ./venv/bin/pytest -q` (нужны проброшенные порты БД/Redis — `make dev-infra`; conftest сам подписывает PASETO dev-ключом, authsvc не нужен; E2E звонков — через in-process fake gRPC + SocketIOTestClient).
...
Swagger UI: `http://localhost:5001/apidocs` (только Flask-эндпоинты — фактически YouGile и presence; REST Go-сервисов в нём нет). Логи: Flask — JSON в stdout (`FLASK_DEBUG=1` включает DEBUG с SQL), Go — slog JSON; в docker — `docker logs` / `make logs`.
