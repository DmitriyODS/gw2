...
func (r *Repo) ListFolders(ctx context.Context, ownerID int64) ([]*domain.Folder, error) {
	rows, err := r.pool.Query(ctx,
		`SELECT `+folderCols+` FROM board_folders f WHERE f.owner_id = $1 ORDER BY f.position, f.name, f.id`, ownerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *Repo) ListChildFolders(ctx context.Context, parentID int64) ([]*domain.Folder, error) {
	rows, err := r.pool.Query(ctx,
		`SELECT `+folderCols+` FROM board_folders f WHERE f.parent_id = $1 ORDER BY f.position, f.name, f.id`, parentID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *Repo) ListSharedRootFolders(ctx context.Context, userID int64, companyIDs []int64) ([]*domain.Folder, error) {
	rows, err := r.pool.Query(ctx, `
		WITH grants AS (
			SELECT folder_id, bool_or(can_edit) AS can_edit FROM (
				SELECT folder_id, can_edit FROM board_folder_user_shares WHERE user_id = $1
				UNION ALL
				SELECT folder_id, can_edit FROM board_folder_company_shares WHERE company_id = ANY($2::bigint[])
			) g GROUP BY folder_id
		)
		SELECT `+folderCols+`, u.fio, u.avatar_path, g.can_edit
		  FROM grants g
		  JOIN board_folders f ON f.id = g.folder_id
		  JOIN users u ON u.id = f.owner_id
		 WHERE f.owner_id <> $1
		   AND NOT EXISTS (SELECT 1 FROM board_folder_recipient_state s WHERE s.user_id = $1 AND s.folder_id = f.id)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&f.ID, &f.OwnerID, &f.ParentID, &f.Name, &f.Color, &f.Position,
			&f.CreatedAt, &f.UpdatedAt, &f.BoardsCount, &f.OwnerName, &f.OwnerAvatar, &canEdit); err != nil {
			return nil, err
		}
...
func (r *Repo) GetFolder(ctx context.Context, id int64) (*domain.Folder, error) {
	rows, err := r.pool.Query(ctx, `SELECT `+folderCols+` FROM board_folders f WHERE f.id = $1`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *Repo) CreateFolder(ctx context.Context, f *domain.Folder) error {
	return r.pool.QueryRow(ctx, `
		INSERT INTO board_folders (owner_id, parent_id, name, color, position)
		VALUES ($1, $2, $3, $4, $5)
...
func (r *Repo) UpdateFolder(ctx context.Context, id int64, name, color string) error {
	_, err := r.pool.Exec(ctx,
		`UPDATE board_folders SET name = $2, color = $3, updated_at = now() WHERE id = $1`, id, name, color)
	return err
}

func (r *Repo) MoveFolder(ctx context.Context, id int64, parentID *int64) error {
	_, err := r.pool.Exec(ctx,
		`UPDATE board_folders SET parent_id = $2, updated_at = now() WHERE id = $1`, id, parentID)
	return err
}

func (r *Repo) DeleteFolder(ctx context.Context, id int64) error {
	_, err := r.pool.Exec(ctx, `DELETE FROM board_folders WHERE id = $1`, id)
	return err
}
...
func (r *Repo) NextFolderPosition(ctx context.Context, ownerID int64, parentID *int64) (int, error) {
	...
	err := r.pool.QueryRow(ctx, `
		SELECT COALESCE(MAX(position), 0) + 1 FROM board_folders
...
func (r *Repo) IsDescendant(ctx context.Context, folderID, maybeAncestor int64) (bool, error) {
	...
	err := r.pool.QueryRow(ctx, `
		WITH RECURSIVE sub AS (
			SELECT id FROM board_folders WHERE id = $2
			UNION ALL
			SELECT f.id FROM board_folders f JOIN sub ON f.parent_id = sub.id
		)
		SELECT EXISTS(SELECT 1 FROM sub WHERE id = $1)`, folderID, maybeAncestor).Scan(&found)
	return found, err
}

// ReparentChildren — перевесить прямых детей папки (подпапки и доски) на
...
func (r *Repo) ReparentChildren(ctx context.Context, folderID int64, newParent *int64) error {
	...
	if _, err := tx.Exec(ctx,
		`UPDATE board_folders SET parent_id = $2 WHERE parent_id = $1`, folderID, newParent); err != nil {
		return err
	}
	if _, err := tx.Exec(ctx,
		`UPDATE boards SET folder_id = $2 WHERE folder_id = $1`, folderID, newParent); err != nil {
		return err
	}
...
func copyFolderRec(ctx context.Context, tx pgx.Tx, ownerID, srcID int64, parentID *int64) (int64, error) {
	...
	if err := tx.QueryRow(ctx, `
		INSERT INTO board_folders (owner_id, parent_id, name, color, position)
		SELECT $1, $2, name, color, position FROM board_folders WHERE id = $3
	...
	for boardRows.Next() {
		...
		srcBoardIDs = append(srcBoardIDs, id)
	}
	boardRows.Close()
...
func (r *Repo) markSharedFolders(ctx context.Context, folders []*domain.Folder) {
	...
	rows, err := r.pool.Query(ctx, `
		SELECT folder_id FROM board_folder_user_shares WHERE folder_id = ANY($1::bigint[])
		UNION
		SELECT folder_id FROM board_folder_company_shares WHERE folder_id = ANY($1::bigint[])`, ids)
	if err != nil {
		return
	}
	defer rows.Close()
...
func (r *Repo) SetFolderRecipientPlacement(ctx context.Context, userID, folderID int64, parentID *int64) error {
	_, err := r.pool.Exec(ctx, `
		INSERT INTO board_folder_recipient_state (user_id, folder_id, parent_id, archived)
		VALUES ($1, $2, $3, FALSE)
...
func (r *Repo) ListRecipientFolders(ctx context.Context, userID int64, companyIDs []int64) ([]*domain.Folder, error) {
	rows, err := r.pool.Query(ctx, `
		...
		SELECT f.id, f.owner_id, st.parent_id, f.name, f.color, f.position,
		  ...
		  JOIN board_folders f ON f.id = g.folder_id
		  JOIN users u ON u.id = f.owner_id
		  JOIN board_folder_recipient_state st ON st.user_id = $1 AND st.folder_id = f.id
