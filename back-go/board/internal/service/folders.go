...
func (s *Service) CreateFolder(ctx context.Context, userID int64, name, color string, parentID *int64) (*domain.Folder, error) {
	...
	if color != "" && !domain.BoardColors[color] {
	...
	s.bus.Publish(ctx, "board_folder:created", []string{userRoom(userID)}, folderPayload(f))
	return f, nil
}
...
func (s *Service) UpdateFolder(ctx context.Context, userID, id int64, name, color string) (*domain.Folder, error) {
	...
	s.publishFolder(ctx, "board_folder:updated", f)
	return f, nil
}
...
func (s *Service) MoveFolder(ctx context.Context, userID, id int64, parentID *int64) (*domain.Folder, error) {
	...
	s.bus.Publish(ctx, "board_folder:updated", []string{userRoom(userID)}, folderPayload(&resp))
	return &resp, nil
}

// DeleteFolder — удалить папку; её прямые дети (подпапки и доски) переезжают в
...
func (s *Service) DeleteFolder(ctx context.Context, userID, id int64) error {
	...
	s.bus.Publish(ctx, "board_folder:deleted", rooms,
		map[string]any{"id": id, "owner_id": userID, "parent_id": f.ParentID})
	return nil
}

// CopyFolder — глубокая копия папки со всем поддеревом и досками (в тот же
...
func (s *Service) CopyFolder(ctx context.Context, userID, id int64) (*domain.Folder, error) {
	...
	s.bus.Publish(ctx, "board_folder:created", []string{userRoom(userID)}, folderPayload(cp))
	return cp, nil
}
...
func folderPayload(f *domain.Folder) map[string]any {
	p := map[string]any{
		...
		"color": f.Color, "position": f.Position, "boards_count": f.BoardsCount,
