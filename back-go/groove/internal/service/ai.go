...
// Единый стиль Грувика: коротко, по фактам, сухой юмор — никакой
// мотивационной воды и сюсюканья.
const systemPrompt = "Ты — Грувик, питомец-талисман корпоративной платформы Groove Work. " +
	"Отвечай кратко (1-3 предложения). Опирайся ТОЛЬКО на переданные факты и цифры. " +
	"Никакой сладкой мотивационной воды, уменьшительных суффиксов и восклицаний через слово. " +
	"Допустим лёгкий сухой юмор. Если фактов недостаточно — скажи меньше, но не выдумывай. " +
	"Отвечай на русском, без кавычек и преамбул."

// Вероятность комментария Грувика по виду события. Комментируются только
// действительно значимые вехи — на рядовые события бот не отвлекает.
...
var staticPhrases = []string{
	"Принято. 3 грува конвертированы в 12 XP — курс сегодня неплохой.",
	...
	"Грувы усвоены. Сытый я усваиваю XP в полтора раза быстрее — учти.",
...
var petOfflineReplies = []string{
	"Мой разговорный модуль (ИИ) отключён в настройках компании. " +
		"Могу только копить XP и молчать со смыслом.",
	"Без ИИ я разговариваю примерно как табличка «скоро вернусь». Администратор может это исправить.",
	"Слышу тебя, ответить нечем: ИИ компании выключен. Факт дня: это лечится в настройках.",
}
...
func (s *Service) refreshPhrases(ctx context.Context, companyID int64) bool {
	...
	text, err := s.ai.Chat(ctx, companyID, []map[string]any{
		...
		{"role": "user", "content": "Придумай 12 коротких реплик (до 90 символов каждая), " +
			...
			"валютой за выполненную работу. Факты для опоры: кормление стоит 3 грува, " +
			"даёт 12 XP и включает сытость (прямой XP за работу ×1.5 до конца дня). " +
			"Тон: сухо, по делу, лёгкая ирония. Без сюсюканья, восклицаний и эмодзи. " +
...
func botPromptForEvent(event *domain.FeedEvent) string {
	...
	// Промпты только для значимых вех — рядовые события бот не комментирует.
	switch event.Kind {
	case "pet_evolved":
		return "Факт: питомец «" + petName + "» сотрудника " + name + " достиг стадии " +
			payloadNum(p, "stage") + " из 6. Один короткий комментарий (до 140 символов): " +
			"констатируй факт, допустима одна сухая шутка. Без поздравительного пафоса."
	case "raid_won":
		return "Факт: команда закрыла " + payloadNum(p, "target") + " задач и победила " +
			"недельного босса «" + payloadStr(p, "boss") + "». Один короткий комментарий " +
			"(до 160 символов): цифра + сухая констатация победы. Без ликования и лозунгов."
	case "pet_recovered":
		return "Факт: питомец «" + petName + "» сотрудника " + name + " выздоровел — " +
			"хозяин вылечил его работой и заботой. Один короткий комментарий " +
			"(до 140 символов), сдержанно, можно с иронией про больничный."
	}
	return ""
}
...
func (s *Service) wrappedPhrase(ctx context.Context, companyID, userID int64,
	...
	parts := []string{
		"Подведи итог рабочей недели сотрудника одной фразой (до 140 символов): " +
			"только факты из цифр ниже, допустима одна сухая шутка. Без похвал ради похвал.",
	...
	if asInt("units") == 0 && asInt("closed") == 0 {
		parts = append(parts, "Неделя была тихой — зафиксируй это без укора и без утешений.")
	}
	text, err := s.ai.Chat(ctx, companyID, []map[string]any{
		{"role": "system", "content": systemPrompt},
...
var morningMoodHint = map[string]string{
	"sick": "Ты болеешь, потому что хозяин давно не работал. Констатируй это и " +
		"напомни, чем лечишься: юниты и закрытые задачи.",
	"buried": "Задач накопилось много. Назови цифры и предложи начать с одной " +
		"конкретной — без паники и бодрых лозунгов.",
	"reminder": "Часть задач засиделась. Укажи на факт, без морали.",
	"fresh":    "Хвостов нет. Отметь это одной фразой, без фанфар.",
	"weekend": "Сегодня у компании выходной — ни слова про задачи и работу. " +
		"Предложи одну конкретную идею отдыха: прогулка, хобби, фильм, спорт.",
}
...
func (s *Service) morningPhrase(ctx context.Context, companyID, userID int64,
	...
	intro := "Скажи одну утреннюю реплику от первого лица про задачи хозяина: "
	if weekend {
		intro = "Скажи одну реплику от первого лица про выходной хозяина: "
	}
	...
	parts := []string{
		"Ты — питомец " + petName + " сотрудника " + bctx.FirstName + ". " +
			"Приветствие уже показано отдельно — не здоровайся, сразу к делу.",
		intro + "1-2 коротких предложения, до 160 символов, только по фактам ниже. " +
			"Сухо, без канцелярита, допустима одна ироничная ремарка. Без эмодзи.",
...
func (s *Service) petSystemPrompt(ctx context.Context, pet *domain.Pet,
	...
	lines := []string{
		...
		"Твой характер: " + persona.Title + " — " + persona.Hint + ". Держи его в тоне, не пережимая.",
		"Твоя стадия роста: " + stageTitle + ", вид: " + speciesTitle + ".",
		"Ты растёшь от работы хозяина: юниты и закрытые задачи дают XP и грувы, " +
			"грувами тебя кормят (сытость ускоряет рост).",
		"Отвечай кратко (1-3 предложения), по-русски. Опирайся ТОЛЬКО на переданные " +
			"факты и цифры. Никакой сладкой мотивационной воды, уменьшительных " +
			"суффиксов и восклицаний через слово. Допустим лёгкий сухой юмор. " +
			"Если фактов недостаточно — скажи меньше, но не выдумывай. Не стыди и не дави.",
	...
	if pet.SickSince != nil {
		lines = append(lines, "Сейчас ты болеешь (хозяин долго не работал). Можешь "+
			"сухо упомянуть это и факт лечения: юниты, закрытые задачи, забота.")
	}
	if isWeekend(now, s.weekendDays(ctx, pet.CompanyID)) {
		lines = append(lines, "Сегодня у компании выходной: не зови работать и не "+
			"предлагай задачи — поддержи отдых (прогулка, хобби, фильм).")
	}
	if workCtx != nil {
...
func (s *Service) generateDigest(ctx context.Context, companyID int64) bool {
	...
	if isWeekend(time.Now().In(domain.MSK), s.weekendDays(ctx, companyID)) {
		// Выходной: вместо рабочей сводки — короткий пост про отдых.
		lines = []string{"Сегодня у команды выходной. Напиши короткий пост для ленты: " +
			"констатируй выходной и предложи одну-две идеи отдыха — прогулка, " +
			"хобби, спорт. Ни слова про задачи и работу. 1-2 предложения, " +
			"до 250 символов, сухо, допустима одна ироничная ремарка."}
	} else {
		dctx := s.digestContext(ctx, companyID)
		lines = []string{"Напиши утренний пост-дайджест для ленты команды: только цифры " +
			"вчерашнего дня из фактов ниже. 1-3 предложения, до 300 символов, " +
			"сухо и по делу, допустима одна сухая шутка. Без приветственных " +
			"фанфар и пожеланий «продуктивного дня»."}
		...
		if closed == 0 && received == 0 && hours == 0 {
			lines = append(lines, "Вчера было тихо — зафиксируй факт без упрёков и утешений.")
		}
	}
	text, err := s.ai.Chat(ctx, companyID, []map[string]any{
		{"role": "system", "content": systemPrompt},
