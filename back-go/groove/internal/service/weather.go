...
const (
	...
	// Не чаще одной погодной реплики в 8 часов — Грувик не метеоканал.
	weatherCooldown = 8 * time.Hour
...
// Статичные реплики, если AI выключен: коротко, по факту, с сухой иронией.
var weatherRemarks = map[string][]string{
	"rain": {
		"За окном дождь. Зонт — на выход, грувы не промокают.",
		"Пошёл дождь. Информирую и возвращаюсь к своим делам.",
	},
	"snow": {
		"За окном снег. Снеговика не слеплю — лапки, но факт зафиксирован.",
		"Снег пошёл. Дороги станут хуже, чай — актуальнее.",
	},
	"storm": {
		"За окном гроза. Я под столом, но это тактическое решение.",
		"Гремит. Электронику и планы на улицу лучше придержать.",
	},
	"fog": {
		"За окном туман. Видимость низкая, у задач — по-прежнему хорошая.",
		"Туман. Ничего не видно, докладывать больше нечего.",
	},
	"cleared": {
		"Распогодилось, вышло солнце. Перерыв на улице стал осмысленной опцией.",
		"Солнце вышло. Короткая прогулка окупится.",
	},
	"heat": {
		"За окном жара за 30. Вода ближе, перегрев — только процессору.",
		"Жарко. Пей воду, тень — по обстоятельствам.",
	},
	"frost": {
		"За окном крепкий мороз. Одевайся по погоде, я утеплён по умолчанию.",
		"Мороз ниже −15. Факт неприятный, но проверенный.",
	},
}
...
func (s *Service) weatherRemarkText(ctx context.Context, companyID int64,
	...
	if s.ai.Enabled(ctx, companyID) {
		...
		prompt := "Ты — " + pet.Name + ", виртуальный питомец-Грувик сотрудника " +
			ownerName + ". Факт: " + weatherRemarkHint[kind] + " (" + cur.Desc + ", " +
			formatTempC(cur.TempC) + "). Напиши ОДНУ короткую реплику хозяину в чат " +
			"об этой перемене погоды (до 140 символов): только факт и, по желанию, " +
			"одна сухая ремарка. Без эмодзи. Не зови работать и не упоминай задачи."
...
func (s *Service) weatherPromptLine(ctx context.Context, userID int64) string {
	...
	return "Факт: за окном у хозяина сейчас " + snap.Desc + ", " + formatTempC(snap.TempC) +
		". Ссылайся на погоду только к месту, не в каждой реплике."
}
