...
func NewServer(eps endpoint.Endpoints, users domain.UserReader,
	...
	scoped.Post("/pet/adventure/recall", h.recallAdventure)
	...
	scoped.Post("/house/theme", h.setHouseTheme)
	scoped.Post("/house/pet-pos", h.setHousePetPos)
	scoped.Get("/shop/mystery", h.getMystery)
	scoped.Post("/shop/buy", h.buyItem)
	scoped.Post("/shop/sell", h.sellItem)
	scoped.Post("/shop/buy-species", h.buySpecies)
	scoped.Post("/walk", h.walkPet)
	scoped.Post("/heal", h.healPet)
	scoped.Post("/sleep", h.sleepPet)
	scoped.Post("/bath", h.bathPet)
	...
	scoped.Get("/bank/stats", h.getBankStats)
	scoped.Post("/bank/goals", h.createGoal)
	scoped.Post("/bank/goals/:id<int>/deposit", h.goalDeposit)
	scoped.Post("/bank/goals/:id<int>/withdraw", h.goalWithdraw)
	scoped.Delete("/bank/goals/:id<int>", h.deleteGoal)
	scoped.Get("/installments", h.getInstallments)
	scoped.Post("/installments/:id<int>/pay", h.payInstallment)
	scoped.Post("/bank/funds", h.createFund)
	scoped.Post("/bank/funds/:id<int>/donate", h.donateFund)
	scoped.Post("/bank/funds/:id<int>/close", h.closeFund)
