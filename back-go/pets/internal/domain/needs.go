...
const (
	NeedMax = 100
	// NeedTick — шаг убывания шкал. Полтора часа: уход должен быть ритуалом, а
	// не тушением пожара. Ориентир — полная шкала сытости живёт трое суток,
	// остальные вшестеро дольше: выходные и командировка не должны сами по
	// себе приводить питомца к болезни, за неё отвечает только настоящее
	// невнимание.
	NeedTick = 90 * time.Minute
)
...
// RecoveredNeedFloor — до какого значения поднимаются при выздоровлении ВСЕ
// шкалы, способные уложить в болезнь (не только шкала-виновник). Без этого
// питомец «вылечивался» с нулевой шкалой и тут же заболевал снова:
// PendingAilment срабатывал на том же нуле в ближайший пересчёт — сперва на
// той же шкале, а после её подъёма на соседней, опустевшей за время болезни.
// Половина шкалы — передышка примерно на сутки; уход всё равно нужен.
const RecoveredNeedFloor = 50
