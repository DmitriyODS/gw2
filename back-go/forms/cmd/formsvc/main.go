// formsvc — микросервис форм и опросов Groove Work.
//
// Владеет формами: их структурой (разделы и вопросы двенадцати типов, ветвление
// по ответам, режим теста), приёмом ответов, сводкой, выгрузкой и шарингом.
// Форма принадлежит ЧЕЛОВЕКУ, а коллеги и компании получают её адресно — уровень
// respond и есть назначение с контролем исполнения. Схему таблиц ведёт
// migrate-контейнер (goose, back-go/migrate).
//
// Транспорт: HTTP/Fiber (HTTP_ADDR) — REST /api/forms/* (за nginx). Сокет-события
// клиентам — Redis-канал gw2:forms:events (доставляет gatewaysvc; pushsvc берёт
// оттуда же назначения и напоминания). Приложенные к ответам файлы — общее
// хранилище (наружу /uploads/<key>). Единственный gRPC — контракт владельца
// файлов для раздела «Настройки → Хранилище».
package main
...
import (
	...
	"github.com/DmitriyODS/gw2/back-go/forms/internal/repository/postgres"
	"github.com/DmitriyODS/gw2/back-go/forms/internal/service"
	httptransport "github.com/DmitriyODS/gw2/back-go/forms/internal/transport/http"
...
func main() {
	...
	httpAddr := bootstrap.Env("HTTP_ADDR", ":8109")
	...
	// Квота хранилища — gRPC billingsvc. Пустой адрес выключает проверки,
	...
	defer billing.Close()
	fileStore := records.NewFileStore(storage.FromEnv(log, uploadFolder), "forms").
		WithQuota(billing, "forms")
	...
	uploads := chunkupload.New(pool, fileStore, "forms", log)

	svc := service.New(service.Deps{
		Repo:    repo,
		Users:   users,
		Files:   fileStore,
		Bus:     events.NewPublisher(rdb, log, "gw2:forms:events"),
		Uploads: uploads,
		Log:     log,
	})
	...
	// Напоминания о сроке ответа назначенным.
	...
	// «Настройки → Хранилище» (вложения ответов).
	grpcAddr := bootstrap.Env("GRPC_ADDR", ":9109")
