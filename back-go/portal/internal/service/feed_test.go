...
// ── Системные посты (gRPC CreateSystemPost) ──────────────────────

func TestCreateSystemPost_CreatesAndPublishes(t *testing.T) {
	svc, repo, bus := newTestService()
	p, err := svc.CreateSystemPost(context.Background(), 1, 10, "pet_evolved", "Эволюция!", "Питомец вырос.")
	if err != nil {
		t.Fatalf("CreateSystemPost: %v", err)
	}
	if p.SystemKind == nil || *p.SystemKind != "pet_evolved" {
		t.Fatalf("ожидался system_kind=pet_evolved, получено %v", p.SystemKind)
	}
	if repo.posts[p.ID] == nil {
		t.Fatalf("пост не сохранён")
	}
	if len(bus.events) != 1 || bus.events[0] != "post:new" {
		t.Fatalf("ожидалось событие post:new, получено %v", bus.events)
	}
}

func TestCreateSystemPost_DedupWithinWindow(t *testing.T) {
	svc, _, bus := newTestService()
	first, err := svc.CreateSystemPost(context.Background(), 1, 10, "pet_evolved", "", "Питомец вырос.")
	if err != nil {
		t.Fatalf("первый CreateSystemPost: %v", err)
	}
	// Ретрай хука в 10-минутном окне — идемпотентно возвращает тот же пост,
	// нового события нет.
	second, err := svc.CreateSystemPost(context.Background(), 1, 10, "pet_evolved", "", "Питомец вырос (ретрай).")
	if err != nil {
		t.Fatalf("повторный CreateSystemPost: %v", err)
	}
	if second.ID != first.ID {
		t.Fatalf("дедуп должен вернуть существующий пост %d, получен %d", first.ID, second.ID)
	}
	if len(bus.events) != 1 {
		t.Fatalf("ретрай не должен публиковать второе событие: %v", bus.events)
	}
	// Другой автор или другой kind — не дедупится.
	other, err := svc.CreateSystemPost(context.Background(), 1, 20, "pet_evolved", "", "Другой владелец.")
	if err != nil {
		t.Fatalf("CreateSystemPost другого автора: %v", err)
	}
	if other.ID == first.ID {
		t.Fatalf("пост другого автора не должен дедупиться")
	}
}

func TestCreateSystemPost_Validation(t *testing.T) {
	svc, _, _ := newTestService()
	if _, err := svc.CreateSystemPost(context.Background(), 1, 10, "  ", "", "тело"); err != domain.ErrSystemKindReq {
		t.Fatalf("ожидалась ErrSystemKindReq, получено %v", err)
	}
	if _, err := svc.CreateSystemPost(context.Background(), 1, 10, "pet_evolved", "", "   "); err != domain.ErrPostBodyReq {
		t.Fatalf("ожидалась ErrPostBodyReq, получено %v", err)
	}
}

func TestCreateSystemPost_DisabledCompanyRejected(t *testing.T) {
	svc, _, _ := newTestService()
	svc.users.(*fakeUsers).inactive = map[int64]bool{1: true}
	if _, err := svc.CreateSystemPost(context.Background(), 1, 10, "pet_evolved", "", "тело"); err != domain.ErrCompanyDisabled {
		t.Fatalf("ожидалась ErrCompanyDisabled, получено %v", err)
	}
}
