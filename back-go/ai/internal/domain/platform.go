...
/* ModelChoiceLocked — выбор модели временно закрыт: все работают на модели
   платформы (GPT). Каталог моделей, цены и личная настройка пользователя
   остаются на месте и оживут, когда выбор снова откроют, — на своём ключе
   модель по-прежнему берётся из настроек пользователя, потому что там платит
   он сам и каталог платформы ни при чём. */
const ModelChoiceLocked = true
