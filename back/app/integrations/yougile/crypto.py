...
отдельная переменная от AI_KEY_ENCRYPTION_KEY: компрометация одного секрета
...
обращении (как и в ai_secret), потому что сохранить ключ YG в открытом виде
или потерять его при сбое ключа шифрования недопустимо.
"""
