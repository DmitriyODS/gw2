    ...
    <main class="tv-canvas">
      ...
      <transition name="tv-stage" mode="out-in">
        <section :key="currentSlide.id" class="tv-stage">
          ...
          <!-- GROOVE (зал славы Грувиков) ──────────────────────────── -->
          <div v-else-if="currentSlide.kind === 'groove'" class="tv-groove-wrap">
            <div class="tv-stage-eyebrow">
              <span style="font-size: 1.2em">👾</span>
              <span>{{ currentSlide.heroEyebrow }}</span>
            </div>
            <div v-if="groovePets.length === 0" class="tv-stage-empty">
              Грувики ещё не вылупились — загляните в «Мой Groove»
            </div>
            <ol v-else class="tv-groove-list">
              <li
                v-for="(p, i) in groovePets"
                :key="p.user_id"
              ...
              >
                ...
                <span class="tv-groove-pet">
                  ...
                  <span v-if="p.hat" class="tv-groove-hat">{{ hatEmojiOf(p) }}</span>
                ...
                <span class="tv-groove-stage">{{ stageTitleOf(p) }}</span>
                <div class="tv-groove-bar">
                  <div class="tv-groove-bar-fill" :style="{ '--bar-width': barPercent(p.xp, grooveXpMax) + '%' }"></div>
                </div>
...
const slides = [
  ...
  // 8. Мой Groove • зал славы Грувиков
  {
    id: 'groove-pets', period: 'day', kind: 'groove',
...
// ─── «Мой Groove» для ТВ: топ питомцев + рейд ────────────────────────────
...
const groovePets = computed(() => (grooveData.value?.pets || []).slice(0, 5))
const grooveXpMax = computed(() =>
  Math.max(1, ...groovePets.value.map(p => num(p.xp))))

function petEmojiOf(p) { return petEmoji(p) }
function hatEmojiOf(p) { return SHOP_ITEMS[p.hat]?.emoji || '' }
function stageTitleOf(p) { return PET_STAGES[p.stage] || '' }
...
const asideContent = computed(() => {
  ...
  if (kind === 'groove-raid') {
    const raid = grooveData.value?.raid
...
/* ── Groove-слайд: зал славы Грувиков ─────────────────────────────── */
.tv-groove-wrap {
  display: flex;
  flex-direction: column;
...
.tv-groove-item {
  ...
  animation: tv-row-in 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) both;
  animation-delay: var(--row-delay, 0ms);
}
