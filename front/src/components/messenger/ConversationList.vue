<template>
  ...
  <AppPage
    ...
    :scroll="false"
    flush
    search-compact="narrow"
    @command="onCommand"
    @menu="$emit('toggle-list')"
  >
    <!-- Поиск — в строку названия: колонка списка узка ПО ЗАМЫСЛУ (и на столе
         тоже), поэтому здесь поиск сворачивается в лупу по ширине панели, а не
         только на телефоне, — список чатов начинается на строку выше. -->
