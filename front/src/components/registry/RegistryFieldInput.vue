<template>
  <div class="rfi">
    ...
    <!-- Галочка -->
    <label v-else-if="field.type === 'checkbox'" class="rfi-check">
    ...
    <!-- Картинка / Файл -->
    <div v-else-if="field.type === 'image' || field.type === 'file'" class="rfi-file">
      <div v-if="modelValue?.path" class="rfi-file-cur">
        <RegistryFieldValue :field="field" :value="modelValue" />
        <button class="rfi-file-rm" title="Убрать" @click="emit('update:modelValue', null)">
          <span class="material-symbols-outlined">close</span>
        </button>
      </div>
      <label class="rfi-upload" :class="{ busy: uploading }">
...
.rfi .ctl { width: 100%; padding: 10px 12px; font: inherit; appearance: none; }
.rfi textarea.ctl { resize: vertical; }
.rfi :deep(.p-select),
.rfi :deep(.p-multiselect),
.rfi :deep(.p-datepicker) { width: 100%; }

.rfi-check { display: inline-flex; align-items: center; gap: 8px; cursor: pointer; }

.rfi-file { display: flex; flex-direction: column; gap: 8px; align-items: flex-start; max-width: 100%; }
.rfi-file-cur { display: flex; align-items: flex-start; gap: 8px; max-width: 100%; min-width: 0; }
.rfi-file-cur :deep(.rf-value) { min-width: 0; }
.rfi-file-rm {
...
.rfi-file-rm:hover { background: var(--color-error-container, var(--color-surface-high)); }
.rfi-upload {
...
.rfi-upload.busy { opacity: 0.6; pointer-events: none; }
</style>
