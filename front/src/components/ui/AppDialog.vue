...
/* Шапка — текст и крестик. Крестик прижат к ВЕРХУ: по центру он оказывался
   напротив второй строки подписи и читался как кнопка при ней, а не как
   закрытие диалога (декоративной иконки-плашки, вокруг которой шапка когда-то
   центрировалась, у диалогов давно нет). */
.dlg-header {
  display: flex;
