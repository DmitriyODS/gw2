<template>
  ...
  >
    ...
    <!-- Тело окна — оно же хост модалок раздела: диалоги телепортируются сюда,
         ...
         Раздел живёт в СВОЁМ контейнере, а не прямо в теле окна, и это не
         лишняя обёртка: Teleport оставляет в цели пару пустых текстовых узлов
         даже у закрытой модалки, и они становились соседями корня раздела.
         При смене раздела Vue берёт якорь вставки ДО демонтажа старого (и
         перешагивает ровно один телепорт) — с двумя закрытыми диалогами якорем
         оказывался узел, который тут же удалялся вместе с разделом, а вставка
...
/* Высоту раздел получает от тела окна — ровно как раньше, когда он лежал в
   нём напрямую. Контейнер намеренно не позиционирован: плавающая кнопка и
   маски модалок по-прежнему отмеряются от .win-body. */
.win-view { height: 100%; }
