...
.mstart.tablet {
  ...
  /* Обои под меню размывает подложка: на светлой картинке подписи плиток и
     строки колонки читались плохо. Размывать умеет только плавающий слой —
     стекло ВНУТРИ него уже ничего не размоет (родитель становится backdrop
     root), поэтому колонки здесь плоские и им это не мешает.
     -webkit- перед стандартным: минификатор иначе выкидывает стандартное. */
