  ...
  >
    <div class="ns">
      <p v-if="isFolder" class="ns-hint">
        ...
        Доступ к папке распространяется на все вложенные папки и доски.
      </p>
      ...
      <!-- ── Публичные ссылки (только доска) ── -->
      <template v-if="!isFolder">
        <div class="ns-divider" />
        <section class="ns-section">
          <h4 class="ns-title"><span class="material-symbols-outlined">link</span>Публичные ссылки</h4>
          <p class="ns-note">По ссылке доску увидит любой — даже без входа в систему.</p>
          <div class="ns-create">
...
const props = defineProps({
  ...
  subjectType: { type: String, default: 'board' }, // board | folder
...
function shareUrl(code) { return `${location.origin}/board/${code}` }
...
async function loadMembers() {
  try {
    const data = isFolder.value
      ...
      : await api.getBoardMembers(props.subjectId)
...
async function addUser(u) {
  try {
    const fn = isFolder.value ? api.shareFolderWithUser : api.shareBoardWithUser
...
async function removeUser(m) {
  try {
    const fn = isFolder.value ? api.unshareFolderUser : api.unshareBoardUser
...
async function addCompany(c) {
  try {
    const fn = isFolder.value ? api.shareFolderWithCompany : api.shareBoardWithCompany
...
async function removeCompany(m) {
  try {
    const fn = isFolder.value ? api.unshareFolderCompany : api.unshareBoardCompany
