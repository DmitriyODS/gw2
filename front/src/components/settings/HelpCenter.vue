  ...
  ...(hasCompany() ? [{
    ...
    articles: [
      {
        ...
        tips: [
          ...
          'Если в компании включён ИИ, поиск понимает смысл запроса, а не только точные слова.',
        ],
      },
      {
        id: 'units',
      ...
      },
      {
        ...
        steps: [
          'Администратор создаёт реестр и настраивает поля карточки.',
          'Участники добавляют записи — карточка собирается по заданной структуре.',
          'Ищите записи строкой поиска, выгружайте таблицу кнопкой «Экспорт».',
        ],
        tips: [
          'Внешняя ссылка открывает реестр на просмотр без входа в платформу — удобно поделиться с подрядчиком. Ссылку можно отозвать в любой момент.',
          'Удаление поля стирает его данные во всех записях — платформа предупредит.',
        ],
      },
      {
        id: 'notes',
        ...
        tourTarget: 'notes',
        paragraphs: [
          'Заметки — ваше личное пространство, не привязанное к компании. Редактор поддерживает заголовки, форматирование, выделение цветом, списки с чекбоксами, таблицы, картинки и код.',
          'Заметки группируются в папки-фильтры: одна заметка может быть в нескольких группах или ни в одной. Всё сохраняется автоматически.',
        ],
        tips: [
          'Публичная ссылка бывает двух видов: «только просмотр» и «редактирование» — по второй заметку можно править вдвоём.',
          'Заметка экспортируется в текстовый файл, а из .txt-файла можно импортировать новую.',
        ],
      },
      {
        id: 'diaries',
        ...
        tips: [
          'Ежедневником можно поделиться адресно: коллега увидит его во вкладке «Поделились», а с правом отметки — сможет закрывать пункты. Сценарий «руководитель раздаёт список — сотрудник выполняет».',
      ...
      },
      {
        ...
        tips: [
          'Поле карточки может показываться условно — только когда другое поле имеет нужное значение.',
          'Внешняя ссылка открывает календарь на просмотр без входа в платформу.',
        ],
      },
    ],
  }] : [{
    ...
    articles: [
      ...
      },
      {
        ...
        tips: [
          'Ежедневником можно поделиться адресно: коллега увидит его во вкладке «Поделились», а с правом отметки — сможет закрывать пункты.',
          'Публичная ссылка открывает ежедневник на просмотр без входа.',
        ],
      },
    ],
    ...
    articles: [
      {
        ...
        paragraphs: [
          'Встроенный чат: текст, картинки, видео, аудио, документы до 25 МБ за одно вложение, запись экрана прямо из поля ввода.',
          ...
          'Удаление — у себя или у обоих. Мессенджер работает даже без компании — это часть вашего личного аккаунта.',
        ],
        steps: [
          'Откройте «Портал» → вкладка «Сотрудники» → карточка коллеги → «Написать».',
          'Перетащите файл прямо в окно чата или вставьте скриншот из буфера (Ctrl+V).',
          'Наведите курсор на сообщение для контекстных действий: ответить, переслать, закрепить, удалить.',
        ],
        tips: [
          'Круглая кнопка в правом нижнем углу — плавающий хаб: мини-чат и ИИ-ассистент поверх любого экрана, даже над запущенным юнитом.',
      ...
      },
      {
        ...
        tips: [
          'В звонок можно пригласить гостя ссылкой — он войдёт из браузера, представившись именем, без аккаунта.',
          'Плашка звонка появляется в чате и обновляется по ходу: кто звонил, сколько длилось.',
        ],
      },
      ...(hasCompany() ? [
        {
          ...
          paragraphs: [
            ...
            'Важный пост закрепляется вверху ленты — сразу или на срок до 30 дней. Бейдж на пункте «Портал» показывает непрочитанные посты.',
          ],
          steps: [
            'Нажмите «Написать пост», добавьте заголовок, текст и файлы.',
            'Выберите раздел, чтобы пост попал в нужную тему.',
            'Пост можно переслать коллеге в мессенджер — он придёт карточкой со ссылкой.',
          ],
          tips: [
            'Разделы ленты создаёт администратор компании.',
            'Закрепить пост может его автор или администратор; одновременно закреплено не больше десяти.',
          ],
        },
        {
          id: 'employees',
      ...
      ...(hasCompany() && usesGroove.value ? [{
        ...
        paragraphs: [
          ...
          'Питомцев коллег можно гладить на странице «Грувики» — им опыт и настроение (поглаживание стоит 2 кудоса, до 3 раз в день на питомца). Там же магазин обликов, кудо-банк, рейтинг недели и питомцы всей команды.',
        ],
        tips: [
          'Грувик может заболеть, если долго не работать (выходные компании не считаются). Лечится юнитами, задачами, прогулкой и заботой коллег.',
          'В магазине бывают ротационные и лимитированные товары, а раз в день доступен бесплатный «сюрприз».',
          'Плавающий питомец в углу экрана перетаскивается в любое место — тап открывает его карточку.',
        ],
      }] : []),
      ...(hasCompany() ? [{
        id: 'assistant',
        title: 'ИИ-ассистент',
        subtitle: 'Ответы по данным компании в плавающем хабе',
        icon: 'smart_toy',
        tone: 'primary',
        tourTarget: 'assistant',
        paragraphs: [
          'Ассистент живёт во вкладке плавающего хаба (кнопка в правом нижнем углу) и отвечает на деловые вопросы по реальным данным компании: сводка часов, отделы, топ сотрудников, календарь загрузки, поиск задач с прямыми ссылками.',
          'Под ответом видно, на какие данные он опирался. Ответ можно оценить 👍 или 👎 — это помогает делать ассистента точнее.',
        ],
        tips: [
          'Работает, когда в компании включён и настроен ИИ (раздел настроек компании).',
          'Спросите «сколько мы отработали на этой неделе?» или «найди задачу про монтаж ролика».',
        ],
      }] : []),
    ],
  },
    ...
    articles: [
      ...(hasCompany() ? [{
        ...
        tips: [
          ...
          'ТВ-режим — живое табло для офисного экрана: слайды с лидерами дня, пульсом недели и рейтингами; состав и длительность слайдов настраиваются шестерёнкой.',
        ],
      }] : []),
      ...(canManageCompanies() ? [{
        id: 'companies',
        ...
        tips: [
          'Управлять участниками и ролями может создатель компании или супер-админ; администратор-не-создатель правит только настройки.',
          'Роль в компании: Сотрудник — работа с задачами, Менеджер — плюс чужие юниты, списки и экспорт, Администратор — плюс настройки компании.',
        ],
      }] : []),
      ...(isAtLeast(ROLES.MANAGER) ? [{
        ...
        paragraphs: [
          ...
          'Создавать и редактировать — с уровня «Менеджер»; списки привязаны к активной компании сессии.',
        ],
        tips: [
    ...
    articles: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          'Режим оформления — светлый, тёмный или системный (следует за настройкой устройства) — переключается отдельным сегментом. Своя сохранённая тема появляется в списке «Мои темы».',
        ],
        tips: [
    ...
    articles: [
      {
        ...
        tips: [
          ...
          'Архив включает аватары и вложения — может быть большим.',
        ],
      },
    ],
  }] : []),
