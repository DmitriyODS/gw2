...
export function registerRegistrySocketHandlers(socket) {
  ...
  // Состав доступа изменился: реестр мог появиться в списке или исчезнуть из него.
  socket.on('registry:shared', (p) => useRegistriesStore().applyRegistrySocket('shared', p))
  ...
  // Учётный реестр: позицию выдали, продлили или вернули — плашка состояния.
  socket.on('record:issue', (p) => useRegistriesStore().applyRecordSocket('issue', p))
}
