...
SVCS="callsvc authsvc msgsvc aisvc petsvc tasksvc gatewaysvc pushsvc mailsvc registrysvc formsvc schedulesvc calendarsvc diarysvc portalsvc notesvc boardsvc drivesvc remindersvc billingsvc alicesvc"
...
cleanup() {
    ...
    if [ -n "$SCHEDULE_PID" ]; then kill -TERM -- "-$SCHEDULE_PID" 2>/dev/null || true; fi
    ...
    if [ -n "$SCHEDULE_PID" ]; then kill -KILL -- "-$SCHEDULE_PID" 2>/dev/null || true; fi
...
for mod in pkg migrate calls auth messenger ai pets tasks gateway push mail registry forms schedule calendar diary portal notes board drive reminder billing alice; do
    printf "  %s" "$mod"
...
# 12aa. Go-микросервис расписаний schedulesvc (HTTP :8110 — REST /api/schedules/*).
#      Файлов раздел не держит, межсервисных вызовов нет: проверка токенов
#      локальная (PASETO_PUBLIC_KEY).
printf "\033[1m▶ schedulesvc (Go)  HTTP :8110...\033[0m\n"
(
  cd "$ROOT/back-go/schedule" && \
  DATABASE_URL="postgresql://grovework:grovework_local@localhost:5432/grovework" \
  REDIS_URL="redis://localhost:6379/0" \
  PASETO_PUBLIC_KEY="$PASETO_PUBLIC_KEY_DEV" \
  HTTP_ADDR=":8110" \
  exec go run ./cmd/schedulesvc
) &
SCHEDULE_PID=$!
...
printf "  Расписание: \033[4mhttp://localhost:8110/api/schedules\033[0m\n"
